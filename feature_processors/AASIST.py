...
class AASIST(nn.Module, BaseProcessor):
    """
    Audio Anti-Spoofing using Integrated Spectro-Temporal Graph Attention Networks.
    Implemented by NAVER corp., based on the paper: https://arxiv.org/pdf/2110.01200
    Distributed under the MIT License.
    Code adapted from: https://github.com/TakHemlata/SSL_Anti-spoofing/blob/main/model.py
    """
    def __init__(self, device="cuda" if torch.cuda.is_available() else "cpu", inputs_dim=1024, compression_dim=128, outputs_dim=1024):
        super().__init__()
        self.device = device
        self.inputs_dim = inputs_dim
        self.compression_dim = compression_dim  # compression dim is hardcoded for the moment
